...
source ~/catkin_ws_raspimouse/devel/setup.bash || { echo "catkin_ws_raspimouse is not working correctly"; exit 1; }

roscd && \
	[[ -z $(pwd | grep opt) ]] && cd ../src || { echo "catkin_ws_raspimouse is not working correctly"; exit 1; }
[ -e "raspimouse_ros" ] && { cd raspimouse_ros; git pull; } || git clone https://github.com/ryuichiueda/raspimouse_ros.git
roscd && cd .. && source ~/catkin_ws_raspimouse/devel/setup.bash && catkin_make
...
source ~/catkin_ws_raspimouse/devel/setup.bash
